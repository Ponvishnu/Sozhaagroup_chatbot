...
COMPANY_URLS = [
    "https://sozhaagroup.com/",
    "https://sozhaagroup.com/about",
    "https://sozhaagroup.com/services",
    "https://sozhaagroup.com/careers",
    "https://sozhaagroup.com/contact",
]

STORAGE_DIR = "chat_data"
TRANSCRIPT_EXCEL = os.path.join(STORAGE_DIR, "sozhaa_group_chat_history.xlsx")
TRANSCRIPT_JSON = os.path.join(STORAGE_DIR, "sozhaa_group_transcripts.json")
...
def fetch_snippets(urls, chars=1500):
    snippets = []
    headers = {"User-Agent": "SozhaaGroupBot/1.0 (+https://sozhaagroup.com)"}
    for u in urls:
        try:
...
def build_system_prompt(snippets):
    ...
    return (
        "You are Sozhaa Group AI Assistant. Use only the company's information provided below "
        "(sozhaagroup.com). Answer only about the company, services, careers, and contact information. "
        "If asked outside scope, reply politely that you only provide Sozhaa Group info. "
...
def build_html_email(user_details, service, transcript):
    header = f"""
    <h2>Sozhaa Group — Chat Transcript</h2>
...
@app.get("/")
def root():
    return {"status": "ok", "msg": "Sozhaa Group Chatbot backend running"}
...
async def chat_endpoint(payload: ChatPayload, background_tasks: BackgroundTasks):
    ...
    if "[User ended the chat]" in user_msg:
        assistant_text = "✅ Thank you for chatting with Sozhaa Group 🚀<br>Our team will contact you soon."
        ...
        if user_phone:
            thank_msg = "✅ Thanks for contacting Sozhaa Group. Our team will contact you soon 🚀"
        ...
        if payload.user_details.get("email"):
            thank_you_html = html + "<br><br><p>🙏 Thank you for chatting with Sozhaa Group. Our team will connect with you soon.</p>"
    ...
    if "support" in user_msg.lower() or "contact" in user_msg.lower():
        ...
        def support_alert():
            try:
                ...
                """
                send_email_with_attachment(COMPANY_EMAIL, "⚠ Sozhaa Group — Support Request", alert_html)
                if payload.user_details.get("email"):
                    send_email_with_attachment(payload.user_details["email"], "Sozhaa Group — Support Request Received", "<p>We received your request. Our team will contact you soon 🚀</p>")
            except Exception as e:
    ...
    def save_and_email():
        ...
        if payload.user_details.get("email"):
            send_email_with_attachment(payload.user_details["email"], "Sozhaa Group — Your Chat Transcript", html, TRANSCRIPT_EXCEL)
